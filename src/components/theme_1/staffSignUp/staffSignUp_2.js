...
//   };
  ...
  setOtherExp = (e) => {
    ...
    if (e.target.value.length < 51) {
      ...
      if (e.target.value.length > 0) this.setState({ error: { ...error } });
...
//   };
  ...
  continueToNext = () => {
    // console.log("clicked inside continuetonext before validation");
    // var state = this.state;
    // var medical = state.exp === "" ? "block" : "none";
    // var nurseLicence = !state.nurseLicence.length ? "block" : "none";
    // var licenseData =
    //   state.nurseLicence.length && !state.nurseLicence[0].licences.length
    //     ? "block"
    //     : "none";
    // // var certifications = !this.props.cert.length ? "block" : "none";
    // // var degree = this.props.degree === "" ? "block" : "none";
    // // var insurance = this.props.insurance === "" ? "block" : "none";
    // var temp = {
    //   medical: medical,
    //   nurseLicence: nurseLicence,
    //   licenseData: licenseData,
    // };
    // this.setState({ error: { ...temp } });

    // if (this.state.exp !== "" && this.state.nurseLicence.length) {
    //   console.log("clicked inside continuetonext in if loop ");
            ...
            <div className="ssu1_body">
              ...
              {this.state.nurseLicence.map((item) => {
                return (
                  <NursingState
                    item={item}
                    setCurState={this.setCurState}
                    key={item.state}
                    stateOfUs={this.state.stateOfUs}
                  />
                );
              })}
              <div
              ...
              >
                <ErrorState
                  show={this.state.error.licenseData}
                  name="Please add your nursing license details."
                />
              </div>
              <div className="ssu2_modal1_input" style={{ maxWidth: "800px" }}>
                ...
                {!this.state.nurseLicence.length && (
                  <div
                    style={{
                      width: "100%",
                      display: "flex",
                  ...
                  >
                    <ErrorState
                      show={this.state.error.nursingLicence}
              ...
              <div
                ...
                style={{ display: this.state.insurance ? "none" : "block" }}
                onClick={() => $("#modal2").show()}
              >
                {" "}
              ...
              {/* <div style={{ width: '100%', display: 'flex', justifyContent: 'center' }}>
                                <ErrorState show={this.state.error.liability} name="Please add your liability insurance." />
                            </div> */}
              <div className="ssu2_text3_4">
                What’s your highest education level?
              </div>
              <Education />
              <div
                className="ssu2_addItem"
                style={{ display: this.state.degree ? "none" : "block" }}
                onClick={() => $("#modal3").show()}
              >
                {" "}
                + Add Degree{" "}
              ...
              {/* <div style={{ width: '100%', display: 'flex', justifyContent: 'center' }}>
                                <ErrorState show={this.state.error.education} name="Please add your highest education level." />
                            </div> */}
              <div className="ssu2_text3_4">
              ...
              {/* <div style={{ width: '100%', display: 'flex', justifyContent: 'center' }}>
                                <ErrorState show={this.state.error.certifications} name="Please add certifications." />
                            </div> */}
              <div className="ssu2_text3_4">
              ...
              <div className="row ssu1_locs">
                {this.state._exps.map((item) => {
                  return (
                    <ToggleExpItem
                      ...
                      exp={item}
                      id={item}
                      toggleExp={this.toggleExp}
                      display="block"
                      selectedExp={this.state.exp}
                    />
                  );
                })}
                <div className="ssu2_otherItem">
                  ...
                  <ToggleExpItem
                    exp="Other"
                    toggleExp={this.showOtherExp}
                    selectedExp={this.state.exp}
                    display={this.state.showOtherExp}
                  />
                </div>
              </div>
              <input
                ...
                style={{
                  display:
                    this.state.showOtherExp === "none" ? "block" : "none",
                }}
                onChange={this.setOtherExp}
                onKeyUp={(e) => {
