...
class StaffSignUp extends React.Component {
  constructor(props) {
    ...
    this.state = {
      // redo: [],
      redo: Array.isArray(props.redo) ? [...props.redo] : [],
      startDate: false,
      error: "none",
      natureErr: "none",
      scheduleErr: "none",
  ...
  continueToNext = async () => {
    var natureErr =
      this.state.permanentPos === false && this.state.tempPos === false
        ? "flex"
        : "none";
    var scheduleErr =
      this.state.fullTime === false && this.state.partTime === false
        ? "flex"
        : "none";
    var weekendErr = this.state.availableWeekEnd === "" ? "flex" : "none";
    var error =
      this.state.dateSelected === false || this.state.startDate === ""
        ? "flex"
        : "none";
    if (
      natureErr === "flex" ||
  ...
  //   // Ensure that 'redo' is properly initialized as an array
  //   var redo = Array.isArray(this.props.redo) ? [...this.props.redo] : [];
            ...
            <div className="ssu_body" style={{ maxWidth: "1100px" }}>
              ...
              <div className="ssu_selBox1">
                <button
                  className={
                    "ssu_sel1 " +
                    (this.state.permanentPos ? "ssu_sel_selected" : "")
                  }
                  onClick={() =>
                    this.setState({
                      permanentPos: !this.state.permanentPos,
                ...
                <button
                  className={
                    "ssu_sel1 " + (this.state.tempPos ? "ssu_sel_selected" : "")
                  }
                  onClick={() =>
                    this.setState({
                      tempPos: !this.state.tempPos,
              ...
              <div
                className="ssu_selBox1"
                style={{ marginTop: 20, display: this.state.natureErr }}
              >
                <ErrorState
                  show="block"
                  name="Please Select At Least One Option from the Nature of Job."
                />
              </div>
              ...
              <div className="ssu_selBox1">
                <button
                  className={
                    "ssu_sel1 " +
                    (this.state.fullTime ? "ssu_sel_selected" : "")
                  }
                  onClick={() =>
                    this.setState({
                      fullTime: !this.state.fullTime,
                ...
                >
                  Full Time <br /> (8hrs or more per day)
                </button>
                <button
                  className={
                    "ssu_sel1 " +
                    (this.state.partTime ? "ssu_sel_selected" : "")
                  }
                  onClick={() =>
                    this.setState({
                      partTime: !this.state.partTime,
                ...
                >
                  Part Time <br /> (7hrs or less per day)
                </button>
              </div>
              <div
                className="ssu_selBox1"
                style={{ marginTop: 20, display: this.state.scheduleErr }}
              >
                <ErrorState
                  show="block"
                  name="Please Select Your Preferred Work Schedule."
                />
              </div>
              <div className="ssu_txt2">Are you available on weekends?</div>
              <div className="ssu_selBox1">
                <button
                  className={
                    "ssu_sel1 " +
                    (this.state.availableWeekEnd === true
                      ? "ssu_sel_selected"
                      : "")
                  }
                  onClick={() =>
                ...
                <button
                  className={
                    "ssu_sel1 " +
                    (this.state.availableWeekEnd === false
                      ? "ssu_sel_selected"
                      : "")
                  }
                  onClick={() =>
              ...
              <div
                className="ssu_selBox1"
                style={{ marginTop: 20, display: this.state.weekendErr }}
              >
                <ErrorState
                  show="block"
                  name="Please Select Your Availability on Weekends."
                />
              </div>
              <div className="ssu_txt2">When are you able to start?</div>
              ...
              <div
                className="ssu_datePickerBox ssu2_modal3_selectBox"
                style={{
                  display:
                    this.state.startDate !== false &&
                    this.state.dateSelected === true
              ...
              <div
                className="ssu_selBox1"
                style={{ marginTop: "20px", display: this.state.error }}
              >
                <ErrorState
                  show="block"
                  name="Please Select Your Start Date."
                />
              </div>
            </div>
            <hr className="ssu_hr" />
