...
class ChangePassword extends React.Component {
    ...
    savePassword = async () => {
        ...
        var cpsw = this.state.cpsw === '' ? 'block' : 'none';
        ...
        if(psw==='none'&& cpsw==='none' && repsw==='none' && valid){
            // var res = await this.props.updateDB({
            //     password: this.state.psw
            // });
            var data = {
                oldPassword : this.state.cpsw,
                newPassword : this.state.psw
            }
            var Authorization = 'Bearer '.concat(window.localStorage.getItem('token'));
                var res = await callApi("POST", "v1/LHS/hospital/changePassword", Authorization, data);
   ...
   render() {
    return (
      <div className="theme2">
          <div> 
            <Header className="theme2_container"/>
