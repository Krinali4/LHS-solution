...
class Institution extends React.Component {
    ...
    setein = async(e) => {
        ...
        this.setState({ mein: e.target.value });
        if (e.target.value !== '') {
            var error = this.state.error;
            error.ein = 'none';
            this.setState({ error: error, einMsgError: '' });
        }
       
        var res = await callApi("GET", `v1/LHS/hospital/ein/${e.target.value}`, null);
        if(res.status === 500){
            var error = this.state.error;
            error.ein = 'block';
            this.setState({ error: error, einMsgError : "EIN Number already present" });
        }
        else {
            var error = this.state.error;
            error.ein = 'none';
            this.setState({ error: error, einMsgError : 'EIN is required' });
        }
    }
    formatInput = (e) => {
    ...
    continueToNext = async () => {
        ...
        var ein =  state.error.ein === 'block' || state.mein === '' ? 'block' : 'none';
        ...
        if( name === 'none' && size === 'none' && website === 'none' && ein === 'none'){
            this.modalClose();
            var data = {
            ...
            if(this.props.admin) {
                ...
                var _id = localStorage.getItem('hospitalId');
            }else {
   ...
   render() {
    return (
        <div className="t2_sp_work">
            ...
            {this.state.isAdmin ? '' :     <img alt="icon.svg" src={edit_note} width="18px" style={{marginTop: 4, float: 'right', cursor: 'pointer'}} onClick={this.edit}/>}
            </div>
            <div className="row t2_sp_work_container">
             ...
             {this.props.admin ? '' :    <div className="col-md-6 col-sm-12">                                      
                    <h6 style={{fontSize:14}}> EIN </h6>
                    <h5 className="t2_detail_textbox" title={this.state.ein}>
                         {/* {this.state.ein}  */}
                         <input className="t2_detail_textbox" style={{ border : 'none' }} type='password' value={this.state.ein}/> 
                         </h5>
                </div> }
            </div>
            <div className="w3-modal" style={{display: this.state.modalShow}} >
                <div className="w3-modal-content ssu2_modal1" style={{maxHeight: 600, overflowY: 'auto'}}>
                   ...
                   {this.props.admin ? '' :      <div className="ssu2_modal1_input">
                            <div className="ssu2_modal1_input ssu2_modal3_selectBox">
                                <div className="input_left_icon">
                                    <img alt="img" width="18px" height="15px" src={institutionSize} />
                                </div>
                                <input className="ssu2_modal3_select" placeholder="EIN" type="password" value={this.state.mein} name="size" onKeyDown={ this.formatInput } onChange={this.setein}
                                    onKeyUp={(e)=>{if(e.key === 'Enter') this.continueToNext()}}/>
                            </div>
                            <ErrorState show={this.state.error.ein} name={this.state.einMsgError} />
                        </div> }
