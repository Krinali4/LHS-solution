...
class JobPost extends React.Component {
    ...
    handleCostPerHour = (e) => {
        // var convertToInt = parseInt(e.target.value, 10)
        // console.log(convertToInt);
// this.setState({ cost : convertToInt })
this.setState({ cost: e.target.value });
    ...
    jobPost = async () => {
        this.setState({ postAdd: true })
        console.log(this.state.openPositionsError, "this.state.openPositionsError");
        ...
        if (this.state.description === "")
            this.setState({ descriptionError: "block" });
        if (this.state.medicalSetting === "") {
            await this.setState({ medicalSettingValue: "block" });
        }
        if (this.state.expStartDate === false) {
            var error = this.state.error;
            error.dateStart = 'block';
            this.setState({ error: error });
        }
        if (this.state.openPositions === "" || String(this.state.openPositions) === "0") {
            this.setState({ openPositionsError: "block" });
        }
        if (this.state.contract !== "Permanent Position" && (this.state.duration === "" || this.state.duration === "0")) {
            this.setState({ durationError: "block" });
        }
        if(this.state.contract !== "Permanent Position" && (this.state.cost === '' || this.state.cost === "0")) {
            this.setState({ costError: 'block' });
        }
        else {
            if (this.state.medicalSettingValue === "none" && this.state.selectedStateError === "none" &&
                this.state.jobTitleError === "none" && this.state.descriptionError === "none" && this.state.error.dateStart === "none"
                && this.state.openPositionsError === "none" && this.state.durationError === "none" && this.state.openPositions !== "0"
                && this.state.costError === 'none' ) {
                    this.setState({ showLoader: true })
                    console.log(this.state.costError, 'sssssssssss');
                var passDate = moment(this.state.expStartDate).utcOffset(0).set({hour:0,minute:0,second:0,millisecond:0 }).add(1,'d')
                var data = {
                    ...
                    expectedStartDate: passDate,
                    openPositions: this.state.openPositions,
                    costPerHour: this.state.cost
                }
                if (this.state.contract === "Temporary Position") {
                    data.weekendAvailiblity = this.state.weekend
                }
        ...
        return (
            <div>
                        ...
                        <select className="locationSelect" name="option" onChange={this.handleState}>
                            <option value="" disabled selected > Select</option>
                            {
                                this.state.stateOfUs.map((state) => {
                                    return <option key={state.num} value={state._id}> {state.name} </option>
                                })
                            }
                        </select>
                    </div>
                ...
                <div className="row m-0">
                    ...
                    <div className="col-4 text-center p-0"><ErrorState show={this.state.medicalSettingValue} name="Others is required." /></div>
                </div>
                    ...
                    <div className='row medicalSettings'>
                        <div className='col-md-5 timeDiv'>
                            ...
                            <div className='time'>
                                ...
                                <select className="AM" name="option" onChange={this.handleStartAP}>
                        ...
                        <div className='col-md-5 timeDiv'>
                            ...
                            <div className='time'>
                                ...
                                <select className="PM" name="option" onChange={this.handleEndAP}>
                ...
                <ErrorState show={this.state.costError} name={this.state.cost === "" ? "Please Add Cost Per Hour." : "Cost Per hour can not be zero."} />
                ...
                <p className="modalName PostLocation" style={{ marginTop: "40px" }}> How many hires do you want for this position?* </p>
                <div className="modalSelect">
                    <div className="ssu2_modal3_selectBox modalSelectBox">
                        <input
                            className="theme2_header_search searchText"
                ...
                <div className="row" style={{ marginTop: "40px" }}>
                    ...
                    <div className="col-md-3 modalBTNCHL" onClick={() => {
                        this.jobPost();
                    }}>
