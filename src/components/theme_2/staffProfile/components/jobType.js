...
class JobType extends React.Component {
    ...
    componentDidMount = async() => {
       ...
       if(res.data){
        this.setState({
            natureOfJob : res.data.natureOfJob,
            weekendAvailiblity: res.data.weekendAvailiblity,
            workSchedule: res.data.workSchedule,
            immediatelyStart: res.data.immediatelyStart,
            startWorkdate : new Date(res.data.startWorkDate)

        })}
        if(this.state.natureOfJob === 'Permanent Position'){
            this.setState({ permPos: true})
        }
        if(this.state.natureOfJob === 'Temporary Position'){
            this.setState({ tempPos: true})
        }
    }

    // componentDidUpdate(prevProps) {
    //     if (prevProps !== this.props) {
    //         console.log(this.props.startWorkdate,'gggggggggggggggggggggggggg');
    //       this.setState({
    //         natureOfJob: this.props.natureOfJob,
    //         workSchedule: this.props.workSchedule,
    //         weekendAvailiblity: this.props.weekendAvailiblity,
    //         immediatelyStart: this.props.immediatelyStart,
    //         // startWorkdate : !this.props.immediatelyStart ? new Date(this.props.startWorkdate) : false
    //       })
    //     }
    //   }
    ...
    setNatureOfJob = async(param) => {
        console.log(param);
        var natureOfJob = this.state.natureOfJob;
        console.log(natureOfJob, 'in line 69');
        var index = natureOfJob.indexOf(param);
        if( index > -1){
            natureOfJob.splice(index, 1);
            console.log(natureOfJob, 'in if ');
         }
        else {
            natureOfJob.push(param);
            console.log(natureOfJob, 'in else');
         }  
         if(!natureOfJob.length){
            alert('please select atleast one natureofjob')
            natureOfJob.push(param);
            console.log(natureOfJob);
        } 
        else {
         var data = {natureOfJob }
            var token =  "Bearer " + localStorage.getItem('token')
            var id = window.localStorage.getItem('_id')
            var res = await callApi("PUT", `v1/LHS/staff/update/${id}`, token, data);
            if(res.data) {
                // this.setState({natureOfJob: res.data.natureOfJob});
                this.componentDidMount();
                alert('Nature of Job is updated')
            }
        }
        // this.updateDB({natureOfJob: natureOfJob});
    }

    setWorkSchedule = async(param) => {
        ...
        if( index > -1){
         ...
         if(!workSchedule.length){
            alert('please select atleast one WorkSchedule')
            workSchedule.push(param);
            console.log(workSchedule);
        } 
        else {
         var data = {workSchedule }
            var token =  "Bearer " + localStorage.getItem('token')
            var id = window.localStorage.getItem('_id')
            var res = await callApi("PUT", `v1/LHS/staff/update/${id}`, token, data);
            if(res.data) {
                // this.setState({natureOfJob: res.data.natureOfJob});
                this.componentDidMount();
                alert('Work Schedule is updated')
            }   
        }
    }

    setWeekendAvailability = async() => {
       var data = {
        weekendAvailiblity : !this.state.weekendAvailiblity
       }
        var token =  "Bearer " + localStorage.getItem('token')
        var id = window.localStorage.getItem('_id')
        var res = await callApi("PUT", `v1/LHS/staff/update/${id}`, token, data);
        if (res.data) {
            this.setState({weekendAvailiblity: res.data.weekendAvailiblity});
            this.componentDidMount();
            alert('Weekend Availiblity is updated')
        }
        // this.updateDB({weekendAvailiblity: !this.state.weekendAvailiblity});
    }

    setStartDate = async(param) => {
        var data;
        if(param){
    ...
    updateDB = async (data) => {
        ...
        var id = window.localStorage.getItem('_id')
        var data = {
            // natureOfJob : this.state.natureOfJob,
            // workSchedule : this.state.workSchedule,
            // weekendAvailiblity : !this.state.weekendAvailiblity,
            startWorkDate: this.state.immediatelyStart
        }
        var res = await callApi("PUT", `v1/LHS/staff/update/${id}`, token, data);
   ...
   render() {
    return (
        <div className="t2_sp_work">
         ...
         {this.state.isAdmin ?        <button className={this.state.natureOfJob.indexOf("Permanent Position") > -1 ?"t2_sp_work_item t2_sp_item_selected":"t2_sp_work_item" }
                // onClick={()=>this.setNatureOfJob("Permanent Position")}
              style={{cursor: 'text' }}  >Permanent Positions</button> : 
             ...
             {this.state.isAdmin ?    <button className={this.state.natureOfJob.indexOf("Temporary Position") > -1 ?"t2_sp_work_item t2_sp_item_selected":"t2_sp_work_item" }
               style={{cursor: 'text' }} >Temporary Positions</button>
                :
            ...
            <div className="row t2_sp_work_container">
            {this.state.isAdmin ?   <button className={this.state.workSchedule.indexOf("Full Time") > -1 ?"t2_sp_work_item t2_sp_item_selected":"t2_sp_work_item" }
             style={{lineHeight: '18px', cursor: 'text'}}>Full Time <br/> <b style={{fontSize: '12px', fontWeight: 'normal'}}>(8hrs or more per day)</b></button>
                : 
           ...
           {this.state.isAdmin ?      <button className={this.state.workSchedule.indexOf("Part Time") > -1 ?"t2_sp_work_item t2_sp_item_selected":"t2_sp_work_item" }
             style={{lineHeight: '18px', cursor: 'text'}}>Part Time <br/> <b style={{fontSize: '12px', fontWeight: 'normal'}}>(7hrs or less per day)</b></button>
            : 
            ...
            <div className="row t2_sp_work_container">
            {this.state.isAdmin ?     <button className={this.state.weekendAvailiblity ?"t2_sp_work_item t2_sp_item_selected":"t2_sp_work_item" } style={{width: 100,cursor: 'text'}}
                >Yes</button>
            : 
            ...
            <div className="row t2_sp_work_container">
           {this.state.isAdmin ?      <button className={this.state.immediatelyStart ?"t2_sp_work_item t2_sp_item_selected":"t2_sp_work_item" }
                // onClick={()=>this.setStartDate("Immediately")}
              style={{cursor: 'text'}}  >Immediately</button>
                : 
