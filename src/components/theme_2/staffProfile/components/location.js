...
class JobType extends React.Component {
    ...
    componentDidMount = async() => {
       ...
       if(res.data){
        this.setState({
           getLocation : res.data.otherCities,
        //    locationId: res.data.otherCities._id
    ...
    addNewLoc = async() => {
        ...
        var data = {
            name : this.state.newLoc.name,
            state : this.state.newLoc.state,
            zipCode : this.state.newLoc.zipCode,
        }
        console.log(data, 'in add location for post api');
        var Authorization = 'Bearer '.concat(window.localStorage.getItem('token'));
        var id = window.localStorage.getItem('_id')
        var res = await callApi("POST", `v1/LHS/staff/otherCities/add?userId=${id}`, Authorization, data);
        if(res.data) {
            this.setState({getLocation : res.data.otherCities })
            this.componentDidMount();
            this.clearNewLoc();
        }
    }

    removeOneLoc = async(index) => {
        // e.stopPropagation();
        var id = this.state.getLocation[index]._id
        console.log(id);
        var Authorization = 'Bearer '.concat(window.localStorage.getItem('token'));
        var _id = window.localStorage.getItem('_id')
        var res = await callApi("DELETE", `v1/LHS/staff/otherCities/${id}?userId=${_id}`, Authorization);
        // if(res.data){
            this.setState({getLocation : res.data.otherCities })
            this.componentDidMount();
        // }
    ...
    setOtherLocation = async (e) => {  
        ...
        if(zip.length === 5){    
            ...
            if(res.result === 'OK'){
                ...
                console.log(this.state.newLoc, 'for new location after zipcode');
            } else {
                this.setState({
                    error: true,
   ...
   render() {
    return (
      ...
      {/* {this.state.getLocation && this.state.getLocation.length ?    */}
         <div className="row t2_sp_work_container">
                    {
                      this.state.getLocation &&  this.state.getLocation.map((item,index)=>{
                            return (                                
                              ...
                              {this.state.isAdmin ? '' :   <span className="t2_sp_loc_btn" onClick={() => this.removeOneLoc(index)}> X </span>}
                                </button>
                            )
                        })
                    }
        ...
        {/* //   : ''  } */}
      {this.state.isAdmin ? '' :       <div className="ssu2_addItem" style={{display: this.state.showInput?'none':'block'}}
            onClick={()=>this.setState({showInput: true})}
            // onClick={this.addLicense}
            >
                + Add {this.state.locations.length ? "Another" : ""} Location 
            </div> }
            <div className="ssu1_input" style={{marginTop: 24, display: this.state.showInput?'block':'none'}}>
                <div className="ssu2_modal1_input ssu2_modal3_selectBox">
                    ...
                    <input style={{ width: '100%' }} className="ssu2_modal3_select" placeholder="Zip Code" type="text" value={this.state.newZip} name="curLoc" onChange={this.setOtherLocation}/>
                </div>
